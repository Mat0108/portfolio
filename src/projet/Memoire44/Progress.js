...
const Progress = () => {
    const progress = [
        ...
        },
        {
            ...
            description:"ajout des hexagones et gestion des cas particulier ",
            progress:"60",
        },
        {
            type:"gestion des hexagones",
            name:"ajout de l'environnement desert et pacifique",
            description:"ajout des hexagones et gestion des cas particulier  ",
            progress:"20",
        },
        ...
        {
            type:"Ajout des scénarios",
            name:"ajout des scénarios hiver, desertique et pacifiques ",
            description:"",
            progress:"5",
        },
        {
            ...
            description:"Ajout des scénarios",
            progress:"0",
        },
        {
            type:"divers",
    ...
    return (
        <div className="w-screen h-screen flex flex-col bg-gray-dark relative  pt-2 ">
            ...
            <div className="w-full flex flex-col p-4 bg-gray-dark pb-20 overflow-y-auto overflow-x-hidden custom-scrollbar" id={"Scrollref"}>
                {progress.map((item,pos)=>{
                    return <div className={`w-[80%] flex flex-col pl-[10%]`} key={`progress-${pos}`}>
                        ...
                        <div className={"ml-[5%] text-white mt-2"}>{item.name} : {item.description}</div>
                        <div className={"h-6 ml-[5%] mt-2 bg-dark_grey rounded-full relative"}>
                            {item.progress !== "0"?<div className={`h-6 bg-green rounded-xl ${getW(parseInt(item.progress),true)}`} ></div> :""}
                            <div className={`absolute text-white text-lg px-2 ${item.progress !== "0" ? "mt-[-24px]" :"mt-[-0px]" }`}>{item.progress}%</div>
                        </div>
